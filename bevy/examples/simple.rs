...
use rhyolite_bevy::{Allocator, Queues, QueuesRouter, Swapchain, RenderSystems, SwapchainConfigExt};
...
fn main() {
    ...
    app.world.entity_mut(main_window).insert(SwapchainConfigExt {
...
impl Plugin for RenderSystem {
    ...
    | {
        ...
        let image_buffer = allocator.create_device_buffer_with_data(image.image.as_raw(), vk::BufferUsageFlags::TRANSFER_SRC).unwrap();
        let intermediate_buffer = allocator.create_device_buffer_uninit(image.image.as_raw().len() as u64, vk::BufferUsageFlags::TRANSFER_DST | vk::BufferUsageFlags::TRANSFER_SRC).unwrap();
        let intermediate_buffer2 = allocator.create_device_buffer_uninit(image.image.as_raw().len() as u64, vk::BufferUsageFlags::TRANSFER_DST | vk::BufferUsageFlags::TRANSFER_SRC).unwrap();
