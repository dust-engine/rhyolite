...
struct BLASStore {
    ...
    blas: BTreeMap<Entity, Arc<AccelStruct>>,
}
...
fn build_blas_system<T: BLASBuildMarker>(
    ...
    changes: Query<Entity, (Or<(Added<T::Marker>, Changed<T::Marker>)>, With<BLAS<T>>)>,
    mut query: Query<(T::QueryData, &mut BLAS<T>), With<T::Marker>>,
    ...
    store.deferred_changes.extend(changes.iter());
    ...
    let mut build_geometry_info: Vec<vk::AccelerationStructureBuildGeometryInfoKHR> =
        std::mem::take(&mut store.deferred_changes)
            .into_iter()
            .map(|entity| {
                let (query_data, blas) = query.get_mut(entity).unwrap();
                ...
                let mut accel_struct = AccelStruct::new_blas(
                    allocator.clone(),
