...
impl TimelineSemaphore {
    ...
    pub fn wait_all_blocked<'a>(semaphores: impl Iterator<Item = (&'a Self, u64)>, timeout: u64) -> VkResult<()> {
        let mut device: Option<&Device> = None;
        let semaphores = semaphores.map(|(s, t)| {
            device = Some(&s.device);
            (s, s.semaphore, t)
        });
        let (semaphores, raws, values): (Vec<&TimelineSemaphore>, Vec<vk::Semaphore>, Vec<u64>) = itertools::multiunzip(semaphores);
        if semaphores.is_empty() {
            return Ok(());
        }
        unsafe {
            device.unwrap().wait_semaphores(&vk::SemaphoreWaitInfo {
        ...
        for (&semaphore, &value) in semaphores.iter().zip(values.iter()) {
            semaphore.current_value.fetch_max(value, std::sync::atomic::Ordering::Relaxed);
        }
        Ok(())
    }
}
