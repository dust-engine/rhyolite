use std::{ops::{Deref, DerefMut, Range, RangeBounds}, ptr::NonNull};
...
impl Drop for ManagedBuffer {
    fn drop(&mut self) {
        unsafe {
            match &mut self.mode {
                ManagedBufferMode::Syncronized { device_buffer, device_allocation, host_buffer, host_allocation, .. } => {
                    self.allocator.destroy_buffer(*device_buffer, device_allocation);
...
impl ManagedBuffer {
    pub fn new(allocator: Allocator, size: u64, usage: vk::BufferUsageFlags) -> VkResult<Self> {
        ...
        unsafe {
            ...
            match memory_model {
                Discrete | Bar | ReBar => {
                    let (device_buffer, device_allocation) = allocator.create_buffer(&vk::BufferCreateInfo {
                        size,
                    ...
                    let (host_buffer, host_allocation) = allocator.create_buffer(&vk::BufferCreateInfo {
                        size,
                    ...
                    let host_coherent =  allocator.physical_device().properties().memory_types()[allocation_info.memory_type as usize].property_flags.contains(vk::MemoryPropertyFlags::HOST_COHERENT);
                    Ok(Self {
                        allocator,
                        ptr: NonNull::new(std::ptr::from_raw_parts_mut(allocation_info.mapped_data as *mut u8, size as usize)).unwrap(),
                ...
                Unified | BiasedUnified => {
                    let (buffer, allocation) = allocator.create_buffer(&vk::BufferCreateInfo {
                        size,
                        usage: usage,
                        ..Default::default()
                    }, &vk_mem::AllocationCreateInfo {
    ...
    pub fn flush(&mut self, range: impl RangeBounds<u64>) {
        ...
        match &mut self.mode {
            ...
            ManagedBufferMode::Shared { allocation, .. } => {
                ...
                self.allocator.flush_allocation(allocation, start, size).unwrap();
            },
        }
    }
    pub fn invalidate(&mut self, range: impl RangeBounds<u64>) {
        ...
        match &mut self.mode {
            ...
            ManagedBufferMode::Shared { allocation, .. } => {
                ...
                self.allocator.invalidate_allocation(allocation, start, size).unwrap();
            },
        }
    }
    pub fn sync(&mut self, command_encoder: &mut impl TransferCommands) {
        match &mut self.mode {
            ManagedBufferMode::Syncronized { flushed_ranges, invalidated_ranges, device_buffer, host_buffer, .. } => {
