...
impl Drop for AccelStruct {
    fn drop(&mut self) {
        unsafe {
            self.device.extension::<ash::extensions::khr::AccelerationStructure>().destroy_acceleration_structure(self.raw, None);
        }
    }
}
...
struct AabbBlasBuilder {
    geometries: Vec<(Buffer, usize, vk::GeometryFlagsKHR, u32)>, // data, stride, flags, num_primitives
